...
/** Self-service staff signup (L1–L4). Returns a signed-in session. */
export function remoteRegister(form: {
  name: string;
  email: string;
  password: string;
  level?: string;
  ...
  return callApi<{ token: string; user: RemoteUser }>('register', undefined, form);
}
