  ...
  return (
    <div className="animate-fade-up">
        ...
        {speech.supported && (
          <button
            ...
            className={`absolute right-3.5 top-9 w-11 h-11 rounded-full grid place-items-center transition-all cursor-pointer ${
              speech.listening ? 'bg-bad text-white animate-pulse-ring' : 'bg-ink text-white hover:scale-105'
            }`}
            aria-label={speech.listening ? 'Stop dictating' : 'Dictate with your voice'}
      ...
      <div className="mt-2 flex items-center justify-between text-xs">
        <span className={speech.error ? 'text-bad' : 'text-faint'}>
          {speech.error
            ? speech.error
            : speech.listening
