...
export default function PRDHub({ processes, isAdmin }: PRDHubProps) {
  ...
  useEffect(() => {
    ...
    setLoadError('');
    try {
      // One-time migrate from our earlier patch key if the Nicole key is empty.
      if (window.localStorage.getItem('bp_prd_engines') === null) {
        const legacy = window.localStorage.getItem('bp_prd_engines_v1');
        if (legacy) {
          try {
            const parsed = JSON.parse(legacy);
            if (Array.isArray(parsed) && parsed.length) {
              // legacy shape used Lucide icons in memory; skip — seed fresh instead
            }
          } catch { /* ignore */ }
        }
      }
      const list = loadPrdEngines(window.localStorage);
      setEngines(list);
      setActiveEngine((prev) => (prev && list.some((e) => e.id === prev) ? prev : list[0]?.id ?? ''));
    } catch (err) {
      console.error('PRDHub: failed to load PRD engines', err);
      setLoadError(err instanceof Error ? err.message : 'Failed to load PRD engines.');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleRefresh = () => {
    ...
    try {
      console.log('[PRDHub] syncPrdEnginesLocal request', { processCount: catalogueProcesses.length });
      const result = syncPrdEnginesLocal(window.localStorage, catalogueProcesses);
      console.log('[PRDHub] syncPrdEnginesLocal response', result);
      if (result.created && result.engine) {
        setEngines(result.engines);
        setActiveEngine(result.engine.id);
      } else {
        setEngines(result.engines);
      }
      ...
      console.error('[PRDHub] syncPrdEnginesLocal failed', err);
  ...
  const confirmDelete = () => {
    ...
    setDeleteError('');
    try {
      const next = deletePrdEngineLocal(window.localStorage, id);
      setEngines(next);
