...
/** Self-service signup — staff only (L1–L4). Admin is bootstrap/admin-create only. */
async function apiRegister(
  ...
  level = String(level || 'L4');
  subFunction = String(subFunction || 'All');

  if (!name || !email) throw new Error('Name and email are required.');
  if (!email.includes('@')) throw new Error('Enter a valid work email.');
  if (password.length < 8) throw new Error('Password must be at least 8 characters.');
  if (!LEVELS.includes(level) || level === 'Admin') {
    throw new Error('Self-registration is for L1–L4 only. Ask an Admin for elevated access.');
  }

  const existing = await sql`SELECT username FROM users WHERE lower(email) = ${email} LIMIT 1`;
  if (existing.length > 0) throw new Error('An account with that email already exists — sign in instead.');
  ...
  await sql`
    INSERT INTO users (username, name, email, level, sub_function, password_hash, salt, active)
    VALUES (${username}, ${name}, ${email}, ${level}, ${subFunction}, ${passwordHash}, ${salt}, true)`;
  await logAudit(sql, 'SYSTEM', 'REGISTER', `${username} (${level})`);
  return apiLogin(sql, username, password);
}
...
export async function routeAction(action: string, token: string | undefined, payload: any = {}): Promise<unknown> {
  ...
  switch (action) {
    ...
    case 'register':
      return apiRegister(sql, payload.name, payload.email, payload.password, payload.level, payload.subFunction);
    case 'changePassword':
